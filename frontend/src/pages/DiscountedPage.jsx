...
import { addToCart } from './CartPage'; // Убедитесь, что путь верный
...
export default function DiscountedPage() {
  ...
  useEffect(() => {
    fetch('http://localhost:3333/products/all')
      .then(res => res.json())
      .then(data => setProducts(data.filter(p => p.discont_price != null)));
  }, []);
  ...
  const handleAddToCart = (product) => {
    const cartItem = {
      id: product.id,
      title: product.title,
      price: product.discont_price || product.price, // итоговая цена со скидкой
      image: product.image
    };
    addToCart(cartItem);
  ...
  return (
    <div className="container" style={{
      ...
      <h2 style={{ fontSize: '24px', fontWeight: 'bold', marginBottom: '24px' }}>Discounted Items</h2>
